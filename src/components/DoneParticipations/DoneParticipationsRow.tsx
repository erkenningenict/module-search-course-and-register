...
import { Certificering, Competentie, Cursus, CursusDeelname, Maybe, Sessie, Thema, Vak } from '../../generated/graphql';
...
export default function DoneParticipationsRow(props: IRow) {
  ...
  return (
    <tr key={row.CursusDeelnameID}>
      ...
      <td>{toDutchDate(new Date(row.Cursus.Sessies && row.Cursus.Sessies[0].Datum).toISOString())}</td>
